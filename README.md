...
This branch does **not** merge `graphGPS`, `san-transformer`, or `graphormer` into itself.
Each backbone branch remains the sole source of truth for its own `results/*.json` — they are
never merged here, and this branch's own git history is unrelated to theirs (it was cut from
`master`). What this branch adds on top of `master` is a **snapshot of a pipeline run**:

- `results_all/` — every `results/*.json` from all three backbone branches, copied in by
  `scripts/collect_cross_backbone_results.py` via `git show <branch>:<path>` (so it never
  needs those branches checked out or fetched as full trees), plus everything
  `scripts/aggregate_results.py`, `scripts/paired_bootstrap.py`, and
  `scripts/make_ranking_figures.py` compute from that snapshot
  (`summary_table.csv`, `criterion_b_*.csv`, `kendall_w.csv`, `paired_bootstrap.csv`).
- `figures/` — the combined cross-backbone figures (`scripts/make_ranking_figures.py`,
  `scripts/make_combined_curves_figure.py`) built from `results_all/`.
- `paper_ACL_format.tex` / `.pdf` — the write-up, whose tables (`results_tables.tex`,
  `results_rho.tex`, `results_critb.tex`, `results_headline.tex`) are generated by
  `scripts/make_paper_tables.py` from `results_all/summary_table.csv`, not hand-typed.

**If a backbone branch gets new results** (e.g. a previously-missing seed lands on
`san-transformer`), this branch's `results_all/` goes stale — it is a point-in-time copy, not
a live view. Refresh it by re-running, from this branch, in order:
...
```
then recompile `paper_ACL_format.tex` and re-check every number the prose quotes by hand —
the table/figure generation is automated, the prose interpreting them is not, and a data
change can flip a qualitative claim (it has: an added SAN seed changed which PE the
Peptides-struct "mechanism split" section describes as the outlier).
